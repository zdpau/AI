...
def worker(ps):
    for _ in range(100):
        ...
        # Compute a gradient update. Here we just make a fake update, but in practice this would use a library like TensorFlow and would also take in a batch of data. 计算梯度更新。这里我们只是做一个虚假的更新，但实际上这将使用像TensorFlow这样的库，并且还会接收一批数据。
        grad = np.ones(10)
